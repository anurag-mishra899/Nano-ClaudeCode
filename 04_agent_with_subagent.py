...
WORKDIR = Path.cwd()
SYSTEM_PROMPT, SUBAGENT_SYSTEM_PROMPT = get_prompt(prompt_type='sub-agent',workdir=WORKDIR)
...
@traceable(run_type="tool", name="Sub-Agent")
def run_subagent(prompt: str) -> str:
    sub_messages = [{"role": "user", "content": prompt}]  # fresh context

    if not sub_messages or sub_messages[0].get("role") != "system":
        sub_messages.insert(0, {"role": "system", "content": SUBAGENT_SYSTEM_PROMPT})

    for _ in range(30):  # safety limit
        clean_messages = normalize_messages(sub_messages)
...
@traceable(name="Agent Loop with Sub-Agent")
