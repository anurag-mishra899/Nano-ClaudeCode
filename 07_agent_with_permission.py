...
@traceable(name="Agent Loop with Permission")
def agent_loop(messages: list,perms: PermissionManager):
    ...
    while True:
        ...
        for tc in assistant_msg.tool_calls:
            ...
            decision = perms.check(name, input or {})
            ...
            if decision["behavior"] == "deny":
                ...
                if perms.ask_user(name, args or {}):
                    handler = TOOL_HANDLERS.get(name)
                    output = handler(**(args or {})) if handler else f"Unknown: {name}"
...
if __name__ == "__main__":
    ...
    while True:
        ...
        final_response = agent_loop(history,perms)
